...
if(!empty($_GET['username']) AND empty($_GET['cariid']))
{
	...
	while($a=mysqli_fetch_array($query)){
		...
		$excel->setActiveSheetIndex(0)->setCellValueExplicit('D'.$numrow, $a['no_hp'], PHPExcel_Cell_DataType::TYPE_STRING);
...
{
	$excel->setActiveSheetIndex(0)->setCellValue('A1', "MANDIRI 1 PILIHAN");
	$excel->getActiveSheet()->mergeCells('A1:Q1');
	$excel->getActiveSheet()->getStyle('A1')->getFont()->setBold(TRUE);
	$excel->getActiveSheet()->getStyle('A1')->getFont()->setSize(15);
	$excel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); // Set text center untuk kolom A1
	...
	$excel->getActiveSheet(0)->setTitle("Daftar Nilai PMDP");
	$excel->setActiveSheetIndex(0);

	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	header('Content-Disposition: attachment; filename="Daftar Nilai PMDP.xlsx"');
	header('Cache-Control: max-age=0');
	$write = PHPExcel_IOFactory::createWriter($excel, 'Excel2007');
	$write->save('php://output');
}
