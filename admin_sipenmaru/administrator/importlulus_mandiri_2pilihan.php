 ...
 if(in_array($extension, $allowed_extension)) //check selected file extension is present in allowed extension array
 {
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   for($row=2; $row<=$highestRow; $row++)
   {
    ...
    $status_pmdp = trim(mysqli_real_escape_string($kon, $worksheet->getCellByColumnAndRow(3, $row)->getValue())," ");
    ...
    if(!empty($username)) // if none of the data are empty
    {
      ...
      {
        mysqli_query($kon,"update tb_formulir4 set status_pmdp='$status_pmdp', prodi_lulus='$prodi_lulus' where username='$username'");  
        ?>
        <div class="alert alert-success">Status Jalur Mandiri 2 Pilihan berhasil diubah untuk 
          username : <?php echo $username.' 
          Nama : '.$nama.' 
          Prodi Lulus : '.$prodi_lulus.'
          dengan status <b>'.$status_pmdp.'</b>';?>
        </div>
        <?php
      }
      else
      {
        ...
        <div class="alert alert-danger">Status Jalur Mandiri 2 Pilihan gagal diubah untuk 
          ...
          Nama : '.$nama.' 
          Prodi Lulus : '.$prodi_lulus.'
          dengan status <b>'.$status_pmdp.'</b>. Data tidak ditemukan';?>
        </div>
        <?php
      }
