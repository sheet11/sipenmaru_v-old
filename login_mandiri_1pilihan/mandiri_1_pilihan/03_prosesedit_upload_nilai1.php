...
<?php
	...
	if(empty($nama_file))
	{
		$qr = mysqli_query($kon,"update tb_formulir4 set tanggal_login = '$date'									
	...
	else
	{
		if($tipe_file == "image/jpeg" || $tipe_file == "image/png" || $tipe_file == "image/jpg")
		{
			if($ukuran_file <= 3000000)
			{
				...
				{
					$qr = mysqli_query($kon,"update tb_formulir4 set 	photo1 = '$date-$nama_file'		
												  	where id_formulir= '$_POST[id_formulir]'");
					if($qr)
					{
			...
			{
				echo "<script>alert('Mohon maaf, Gambar tidak boleh melebihi 1 Mb.');window.location='03_upload_nilai.php'</script>";
			}
		}
		else
		{
